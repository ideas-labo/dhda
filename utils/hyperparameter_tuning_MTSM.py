...
def nn_l1_val(X_train1, Y_train1, X_train2, Y_train2, n_layer, lambd, lr_initial, max_epoch):
    config = dict()
...
def hyperparameter_tuning(args=[[],[],[],[],[],[],[]]):
    ...
    print('Step 1: Tuning the learning rate ...')
    temp_config = dict()
    ...
    count = 0
    layer_range = range(n_layer_opt, n_layer_opt+1)
    ...
    for idx, lambd in enumerate(lambda_range):
        val_abserror, val_relerror = nn_l1_val(X_train1, Y_train1,
                                               X_train2, Y_train2,
                                               n_layer_opt, lambd, temp_lr_opt, max_epoch)
