        ...
        for i in range(1, n_layer + 1):
            if i == 1:
                layer = tf.layers.dense(layer, n_neuron, tf.nn.relu,
                                        ...
                                        kernel_regularizer=tf.keras.regularizers.l1(float(lambd)), name=str(i))
            else:
                layer = tf.layers.dense(layer, n_neuron, tf.nn.relu,
                                        kernel_initializer=tf2.initializers.GlorotUniform(seed=1), name=str(i))
